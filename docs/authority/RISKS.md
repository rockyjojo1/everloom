...
|---|---|---|---|---|---|
| Weak-agent false completion | A model calls a feature "complete" from a build, test or file existing, without runtime evidence | Wasted supervisor time, false product confidence | Independent gate audit; translate "complete" into provable claims and test each one | Codex | Runtime-reachable proof matching the claim |
...
| Platform or iPhone failure | No physical-device evidence yet | Wrong engine bet on the primary platform | Physical bake-off before deep Verdant integration (work package 5) | Codex, Owner | Sustained FPS, memory and lifecycle evidence on a real device |
...
| Asset-provenance gaps | Licence/source fields missing or unverified in the registry/manifest | Legal exposure, rework when a source is later found unusable | Read-only provenance audit before any new downloads (work package 2) | Codex, Owner | Every used asset has source, licence evidence, local path and status |
