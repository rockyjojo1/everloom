...
| Platform or iPhone failure | No physical-device evidence yet | Wrong engine bet on the primary platform | Physical bake-off before deep Verdant integration (work package 5) | Codex, Owner | Sustained FPS, memory and lifecycle evidence on a real device. Progress as of Gate 5A (`claude/capacitor-ios-bakeoff`): a committed, statically-verified Capacitor 8 iOS project exists and syncs the authoritative build, with an unsigned Simulator compile passing in CI. Further progress as of Gate 5B (`claude/capacitor-ios-simulator-runtime`): a real XCUITest journey proved the compiled app actually runs inside a booted iOS Simulator — real touch/keyboard entry, HUD interactivity, backgrounding/resume, native process termination/relaunch, and local-save persistence across that termination, CI run [30976961883](https://github.com/rockyjojo1/everloom/actions/runs/30976961883) (1 test, 0 failures). This closes the "does the app even run and hold basic lifecycle/persistence together" gap at the Simulator level — but physical-device evidence itself (real FPS/memory/thermal, real touch feel, real background-suspension behaviour under real memory pressure) is still fully open; do not treat Gate 5A or Gate 5B as closing this risk |
