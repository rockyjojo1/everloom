...
function rowToPlayerState(row: Record<string, unknown>): PlayerState {
  return {
    ...
    activeDeedId: (row["active_deed_id"] as string | null) ?? null,
    completedDeedIds: (row["completed_deed_ids"] as PlayerState["completedDeedIds"]) ?? [],
    deedProgress: (row["deed_progress"] as PlayerState["deedProgress"]) ?? {},
...
function playerStateToRow(ps: PlayerState): Record<string, unknown> {
  return {
    ...
    active_deed_id: ps.activeDeedId,
    completed_deed_ids: ps.completedDeedIds,
    deed_progress: ps.deedProgress,
    pets: ps.pets,
    version: ps.version,
    is_dead: ps.isDead,
