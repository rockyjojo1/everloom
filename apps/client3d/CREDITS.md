# Asset Credits

All assets in this project are licensed under CC0 (Creative Commons Zero) or equivalent public domain licenses.

## Model Packs

### KayKit — Character Pack: Adventurers (Test/Placeholder)
- **Source**: https://kaylousberg.itch.io/kaykit-adventurers
- **Author**: Kay Lousberg
- **License**: CC0
- **Description**: Rigged humanoid characters with animations (Knight, Barbarian, Mage, Rogue). Includes idle, walk, run, attack, interact, death animations and modular weapons/armor.
- **Note**: P0 uses a procedurally-generated test model. Real asset pack download URL requires direct itch.io access which may need authentication.

### KayKit — Skeletons Pack (To Download)
- **Source**: https://kaylousberg.itch.io/kaykit-skeletons
- **Author**: Kay Lousberg
- **License**: CC0
- **Description**: Enemy skeleton variants for dungeon/combat encounters

### KayKit — Dungeon Pack (To Download)
- **Source**: https://kaylousberg.itch.io/kaykit-dungeon
- **Author**: Kay Lousberg
- **License**: CC0
- **Description**: Dungeon props, tileable floor/wall pieces, lighting fixtures

### Kenney — Nature Kit (To Download)
- **Source**: https://kenney.nl/assets/nature-kit
- **Author**: Kenney (kenney.nl)
- **License**: CC0
- **Description**: Nature elements: trees, rocks, water, plants, grass

### Kenney — Fantasy Town Kit (To Download)
- **Source**: https://kenney.nl/assets/fantasy-town-kit
- **Author**: Kenney (kenney.nl)
- **License**: CC0
- **Description**: Town buildings, fences, market stalls, fantasy architecture

### Quaternius — RPG Pack (To Download)
- **Source**: https://quaternius.com
- **Author**: Quaternius
- **License**: CC0
- **Description**: Gap-filler assets: monsters, NPCs, items, environmental props

## Notes

- All assets are verified CC0/public domain as of P0
- Models are expected in GLTF 2.0 (.glb or .gltf + .bin) format
- No Jagex/OSRS game assets, no scraped game data, no pixel art
- Asset packs are organized in `public/models/<pack-name>/` subdirectories

---

## Verified installed packs (supervisor-installed, see ASSET_INVENTORY.md)

- **KayKit Character Pack: Adventurers** — Kay Lousberg — CC0 —
  https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0
- **KayKit Character Pack: Skeletons** — Kay Lousberg — CC0 —
  https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Skeletons-1.0
- **KayKit Dungeon Remastered** — Kay Lousberg — CC0 —
  https://github.com/KayKit-Game-Assets/KayKit-Dungeon-Remastered-1.0
- **Kenney Nature Kit** (330 assets) — Kenney.nl — CC0 —
  https://kenney.nl/assets/nature-kit
- **Kenney Fantasy Town Kit 2.0** (160 assets) — Kenney.nl — CC0 —
  https://kenney.nl/assets/fantasy-town-kit

All CC0 (public domain). No Jagex/RuneScape or other proprietary game assets are
used anywhere in this project.
